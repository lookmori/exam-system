...
export default function ImportTeachersPage() {
  ...
  async function handleImport() {
    ...
    const res = await fetch("/api/users/import", {
      ...
      body: JSON.stringify({ rows: preview.filter((r: any) => !r.error) }),
    });
    const data = await res.json();
    setResult(data);
    setImporting(false);
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <PageHeader title="批量导入教师" description="通过Excel文件批量导入教师账号" />
      </div>
      {result ? (
        <Card><CardContent className="py-12 text-center">
          ...
          <div className="flex justify-center gap-3">
            <Link href="/admin/school/teachers"><Button variant="outline">返回列表</Button></Link>
        ...
        <Card>
          <CardHeader><CardTitle>数据预览 ({validCount} 条有效)</CardTitle></CardHeader>
          <CardContent>
            <div className="overflow-x-auto border rounded-lg max-h-96">
              <table className="w-full text-sm"><thead><tr className="border-b bg-slate-50">
                <th className="p-3 text-left">姓名</th><th className="p-3 text-left">用户名</th><th className="p-3 text-left">角色</th><th className="p-3 text-left">状态</th>
              </tr></thead>
              <tbody>{preview.slice(0, 50).map((row: any, i: number) => (
                <tr key={i} className="border-b last:border-0">
                  <td className="p-3">{row.name || "-"}</td><td className="p-3">{row.username}</td><td className="p-3">{row.role}</td>
                  <td className="p-3">{row.error ? <span className="flex items-center gap-1 text-red-500 text-xs"><XCircle className="h-3 w-3" />{row.error}</span> : <CheckCircle className="h-4 w-4 text-emerald-500" />}</td>
                </tr>
              ))}</tbody>
              </table>
            </div>
            <div className="flex justify-end gap-3 mt-4">
              ...
              <Button onClick={handleImport} disabled={importing || validCount === 0}>{importing ? <Loader2 className="h-4 w-4 animate-spin mr-1.5" /> : null}确认导入 {validCount} 人</Button>
            </div>
          </CardContent>
        </Card>
      ) : (
        <Card><CardContent className="py-12 text-center">
          ...
          <p className="text-sm text-slate-500 mb-6">列：name（姓名，可选）、role（必填）、username（留空自动生成2开头的8位数字）、password（留空默认123456）</p>
          <input type="file" accept=".xlsx,.xls" onChange={e => setFile(e.target.files?.[0] || null)}
            className="block mx-auto mb-3 text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-blue-50 file:text-blue-700" />
